...
if [[ "$1" == apache2* ]] || [ "$1" == php-fpm ]; then
	...
	if [ "$haveConfig" ]; then
		: "${WORDPRESS_DB_HOST:=BancoMySQLWordPress}"
		: "${WORDPRESS_DB_PORT:=3307}"
		: "${WORDPRESS_DB_USER:=WordPress_User_EFAZ2018}"
		: "${WORDPRESS_DB_PASSWORD:=Pass_WordPress_EFAZ2018}"
...
EOPHP
		...
		set_config 'DB_PORT' "$WORDPRESS_DB_PORT"
		set_config 'DB_USER' "$WORDPRESS_DB_USER"
		set_config 'DB_PASSWORD' "$WORDPRESS_DB_PASSWORD"
		set_config 'DB_NAME' "$WORDPRESS_DB_NAME"
		set_config 'DB_CHARSET' "$WORDPRESS_DB_CHARSET"
		set_config 'DB_COLLATE' "$WORDPRESS_DB_COLLATE"
		set_config 'WP_HOME' "http://efaz.sefaz.al.gov.br"
		set_config 'WP_SITEURL' "http://efaz.sefaz.al.gov.br"
